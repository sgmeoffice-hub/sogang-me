...
- **기존 글 영문 백필 완료(9/25)**: 공개 글 2,253건의 영문 제목·본문·요약 6,478칸을 채움(영문이 비어 있던 칸만, 관리자 입력분 덮지 않음). 방법: 본문을 번역 단위 16,835개(709K자)로 나눠(`lib/html-segments`와 같은 분할) Claude 세션 에이전트 15개가 번역 → 검증기(한글 잔존·표지 수·중첩) 통과 → QA(제목 2,227개 전수, 본문 표본 500개, 인물 이름 통일) → 사이트와 같은 후처리(polishEnglish·enforceNames) → DB. QA 관찰 오류율 약 1%, 수정 135건(학번 'class of'→'entering class of', 전기/후기 졸업 표기, [언론보도]→[Media Coverage], 「」→“”, '오전 12시' 마감→noon 등). 외부 인물 표기 통일: 이정철 Jungchul Lee, 김원정 Wonjung Kim, 최범규 Bumkyoo Choi, 공경철 Kyoungchul Kong 등 20명(공개 표기 확인). 태그 집합 100% 보존(순서만 일부 영어 어순에 맞춰 이동). 원문이 빈 글 본문 110·요약 145칸은 ''로 표시.
- **남은 결정(책임자)**: ① 옛 글 25건에 학생·담당자 휴대폰 번호, 3건에 학번으로 보이는 숫자(게시 금지 규칙 위반, 국문 원문에도 있음): 휴대폰 154, 159, 167, 197, 220, 242, 251, 252, 285, 373, 432, 586, 933, 987, 1207, 1659, 1763, 1854, 1962, 1964, 2061, 2249, 2264, 2292, 2366 / 학번류 127, 162, 903 — 마스킹 여부 확인 필요 ② 퇴임·외부 교수(이정철 등)는 교수진 표에 없어 새 글 자동 번역 때 이름 교정이 안 됨 — 필요하면 별칭 표 추가 ③ 우수논문상/최우수논문상 영문 구분(현재 Best/Excellent Paper Award 혼용)
